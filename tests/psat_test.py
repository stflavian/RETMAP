...
PRESSURE_CRITICAL = adsorbate_data["pressure_critical"]  # [MPa]
...
def fugacity_ratio(pressure_guess):
    roots = np.roots(peng_robinson_polynomial(pressure_critical=PRESSURE_CRITICAL,
...
saturation_pressure = fsolve(func=fugacity_ratio, x0=np.array(PRESSURE_GUESS))

print(saturation_pressure[0])
